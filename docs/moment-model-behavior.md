...
## Implications for Archetype AI Model

These findings about MOMENT inform how to approach the Archetype AI time-series model:

1. **Input contract**: Time-series models expect fixed-size numeric tensors, not text tokens.
   Check Archetype AI's expected window size (may differ from 512).

2. **Output contract**: Expect fixed-size numeric output. Determine if Archetype AI
   returns reconstruction, embeddings, or both.

3. **Determinism**: Verify whether Archetype AI's model is deterministic or has
   stochastic components.

4. **Latency profile**: MOMENT's ~600 ms CPU latency sets a baseline. Archetype AI
   may differ based on model size and architecture.

5. **API pattern**: The HTTP POST → JSON response pattern is standard for time-series
   models. Archetype AI likely follows a similar pattern (not streaming).

6. **Edge deployment**: MOMENT runs on CPU with ~2.5 GB RAM. This constrains what
   hardware is needed. Archetype AI's requirements may differ.
